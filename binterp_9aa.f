C
         SUBROUTINE BINTERP(xxin, ijn, yyin, ikn, zzin, xxout, ijo, 
     >                      yyout, iko, iyl, izl, ZZOUT)
C
C   routine to do bilinear interpolation (latitude-height) on model input files, (EF, 8/01)
C
C  
C   NOTE: IF the NEW ARRAY has grid points beyond the native array, THIS ROUTINE will set the values
C         to be constant beyond the native array - this is already built in.
C
C
C
C example CALLING sequence:
C
C        CALL BINTERP(LAT, 37, ZZ, 59, TEMP, LATI, 73, ZZI, 150, 1, 1, TEMPI)
C
C    
C    INPUT:
...
CEF#include "com2d.h"
       ...
       REAL*4 xxin(ijn), yyin(ikn), zzin(ijn,ikn)
       REAL*4 xxout(ijo), yyout(iko), zzout(ijo,iko)
...
c  interpolate in x-axis direction, then y-direction, fix up edges and corners as special cases
     ...
     >       (xxin(ij0+1)-xxin(ij0))*(xxout(ij)-xxin(ij0)) + zzin(ij0,1)
            ...
            zzout(ij,ik) = EXP(ALOG(zzin(ij0+1,1)/zzin(ij0,1))/
     > (xxin(ij0+1)-xxin(ij0))*(xxout(ij)-xxin(ij0))+ ALOG(zzin(ij0,1)))
          endif
        endif
     ...
     >     (xxin(ij0+1)-xxin(ij0))*(xxout(ij)-xxin(ij0)) + zzin(ij0,ikn)
           ...
           zzout(ij,ik) = EXP(ALOG(zzin(ij0+1,ikn)/zzin(ij0,ikn))/
     >(xxin(ij0+1)-xxin(ij0))*(xxout(ij)-xxin(ij0))+ALOG(zzin(ij0,ikn)))
          endif
        endif
     ...
     >            *(xxout(ij)-xxin(ij0)) + zzin(ij0,ik0+1)
          ELSE
          q1 = EXP(ALOG(zzin(ij0+1,ik0)/zzin(ij0,ik0))/(xxin(ij0+1)-
     >           xxin(ij0))*(xxout(ij)-xxin(ij0)) + ALOG(zzin(ij0,ik0)))

          q2 = EXP(ALOG(zzin(ij0+1,ik0+1)/zzin(ij0,ik0+1))/(xxin(ij0+1)-
     >         xxin(ij0))*(xxout(ij)-xxin(ij0)) + ALOG(zzin(ij0,ik0+1)))
          endif
        endif
     ...
     >              (yyout(ik)-yyin(ik0)) + q1
             ...
             zzout(ij,ik) = EXP(ALOG(q2/q1)/(yyin(ik0+1)-yyin(ik0))*
     >            (yyout(ik)-yyin(ik0)) + ALOG(q1))
          endif
      ENDIF
