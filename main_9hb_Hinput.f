...
C       BThomas Dec2022 - flag and start/end times for input of additional H source
	! declared in com2d_9hb_Hinput.h and used in main_9hb.f and solv_9hb_Hinput.f
	OPEN(UNIT=1589, FILE='Hinflag.txt')
        READ(1589,*) Hinflag
	CLOSE(UNIT=1589)     
	print *, "MAIN: Hinflag = ", Hinflag 
	Hinflag_solv = 0

	OPEN(UNIT=1590, FILE='HinStart.txt')
        READ(1590,*) HinStart
	CLOSE(UNIT=1590)
	print *, "MAIN: HinStart = ", HinStart 

	OPEN(UNIT=1591, FILE='HinStop.txt')
        READ(1591,*) HinStop
	CLOSE(UNIT=1591)     
	print *, "MAIN: HinStop = ", HinStop 
C end BThomas Dec2022
...
C       BThomas Dec2022 - Determine if additional H input source will be used:
	Hinflag_solv = 0
	if(Hinflag .eq. 1) then
	   if (runNum .eq. 1) then !start H input on first run set
	      if ((I .ge. HinStart) .and. (I .le. HinStop)) then
		 print *, "Hinput on " 
		 Hinflag_solv = 1   
	      end if
	   end if
	end if
	if(runNum .gt. 1) Hinflag_solv = 0 !Here you can have it limited to just first run, or do a steady-state input over the full set of runs (in that case set =1)
	if (Hinflag_solv .eq. 0) print *, "Hinput OFF"
