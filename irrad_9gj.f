...
c  $Id: irrad.f,v 1.33 2007/04/20 16:20:54 ltakacs Exp $

      subroutine irrad (m,np,plem,ta,wa,oa,tb,co2,
...
!     *                  aerosol,na,taual,ssaal,asyal,
     *                  flx,flc,fla,dfdts,sfcem,taudiag)
...
c
c   (1) A layer was added above ple(1) to account for the downward  
...
c   level pressure (plem)                             mbar    m*(np+1)
...
c   n2o mixing ratio by volume (n2o)                  pppv    m*np
c   ch4 mixing ratio by volume (ch4)                  pppv    m*np
c   cfc11 mixing ratio by volume (cfc11)              pppv    m*np
c   cfc12 mixing ratio by volume (cfc12)              pppv    m*np
c   cfc22 mixing ratio by volume (cfc22)              pppv    m*np
...
c---- output parameters
c
c   net downward flux, all-sky   (flx)               W/m^2   m*(np+1) (original code listed these as fraction,
c   net downward flux, clear-sky (flc)               W/m^2   m*(np+1)     but they should be in W/m^2)
c   net downward flux, clear-sky no aerosol (fla)    W/m^2   m*(np+1)
...
c   (4) ple(1) is the pressure at the top of the model atmosphere,
c        and ple(np+1) is the surface pressure.
...
c---- input parameters ------
      ...
      real plem(m,np+1), ple(m,np+1), ta(m,np),wa(m,np),oa(m,np),tb(m)
      real n2o(m,np),ch4(m,np),cfc11(m,np),cfc12(m,np),cfc22(m,np)
      real fs(m,ns),tg(m,ns),eg(m,ns,10),co2(m,np)
      real tv(m,ns),ev(m,ns,10),rv(m,ns,10)
      real cwc(m,np,3),fcld(m,np)
...
c---- output parameters ------
      ...
      real flx(m,np+1),flc(m,np+1),fla(m,np+1),dfdts(m,np+1),sfcem(m)
...
c---- temporary arrays -----
      ...
      real pa(m,0:np),dt(m,0:np),tx(m),xlayer(m)
      real reff(m,np,3),taucl(m,np,3)
      real x1(m),x2(m),x3(m)
      real dh2o(m,0:np),dcont(m,0:np),dco2(m,0:np),do3(m,0:np)
      real dn2o(m,0:np),dch4(m,0:np)
      real df11(m,0:np),df12(m,0:np),df22(m,0:np)
      real th2o(m,6),tcon(m,3),tco2(m,6,2)
      real tn2o(m,4),tch4(m,4),tcom(m,6)
      real tf11(m),tf12(m),tf22(m)
      real h2oexp(m,0:np,6),conexp(m,0:np,3),co2exp(m,0:np,6,2)
      real n2oexp(m,0:np,4),ch4exp(m,0:np,4),comexp(m,0:np,6)
      real f11exp(m,0:np),f12exp(m,0:np),f22exp(m,0:np)
      real blayer(m,0:np+1),blevel(m,0:np+1)
      real cd(m,0:np+1),cu(m,0:np+1)
      real bd(m,0:np+1),bu(m,0:np+1)
      real ad(m,0:np+1),au(m,0:np+1)
      real bs(m),dbs(m),rflxs(m)
      real dp(m,0:np),cwp(m,np,3)
      real taant(m,0:np+1)
      real trant(m,0:np+1),tranal(m)
      real transfc(m,0:np+1),trantcr(m,0:np+1),trantca(m,0:np+1)
      real flau(m,0:np+1),flad(m,0:np+1)
      real flcu(m,0:np+1),flcd(m,0:np+1)
      real flxu(m,0:np+1),flxd(m,0:np+1)
      real taua(m,np),ssaa(m,np),asya(m,np),taerlyr(m,0:np)
      real taual,ssaal,asyal
!bdc
      real w11,p11,w12,p12,w13,p13
!bdc
!mjs
      integer ncld(m,3)
      integer icx(m,0:np)
...
!mjs
      ...
      real cldhi(m),cldmd(m),cldlw(m),tcldlyr(m,0:np),fclr(m,0:np+1)
      real qaero(m,np,na)
...
c     range of co2 amount (up to 100 times of the present-time value).
      ...
      include "o3.tran3"
!bdc
      data w11, w12, w13, p11, p12, p13,dwe,dpe/-8.0, -4.0, -6.0,
     1 3*-2.0, 3.33333,5.0/
!bdc


CCEF - first convert input pressure levels from mbar to Pascal here,  plem, ple(m,np+1)
C
      do k=1,np+1
       do i=1,m
         ple(i,k) = plem(i,k)*100.
       enddo
      enddo


c-----compute layer pressure (pa) and layer temperature minus 250K (dt)
Ccef                        - the IRRAD code assumes ple is in Pascals here, the .01 factor converts to mbar

      do k=1,np
       do i=1,m
         pa(i,k) = 0.5*(ple(i,k+1)+ple(i,k))*0.01
         dp(i,k) =     (ple(i,k+1)-ple(i,k))*0.01
         dt(i,k)=ta(i,k)-250.0
       enddo
      enddo
...
c     the factor 6.081 is equal to 1800/296
      ...
      do k=1,np
       do i=1,m
         ...
         dco2 (i,k) = 789.*co2  (i,k)*dp(i,k)
         dch4 (i,k) = 789.*ch4  (i,k)*dp(i,k)
         dn2o (i,k) = 789.*n2o  (i,k)*dp(i,k)
         df11 (i,k) = 789.*cfc11(i,k)*dp(i,k)
         df12 (i,k) = 789.*cfc12(i,k)*dp(i,k)
         df22 (i,k) = 789.*cfc22(i,k)*dp(i,k)
...
Ccef                                 - this assumes array ple is in Pascals here, .01 factor converts to mbar

       do i=1,m
         dp   (i,0) = max(ple(i,1)*0.01,0.005)
         pa   (i,0) = 0.5*dp(i,0)
         dt   (i,0) = ta(i,1)-250.0

         dh2o (i,0) = 1.02*wa   (i,1)*dp(i,0)
         do3  (i,0) = 476.*oa   (i,1)*dp(i,0)
         dco2 (i,0) = 789.*co2  (i,1)*dp(i,0)
         dch4 (i,0) = 789.*ch4  (i,1)*dp(i,0)
         dn2o (i,0) = 789.*n2o  (i,1)*dp(i,0)
         df11 (i,0) = 789.*cfc11(i,1)*dp(i,0)
         df12 (i,0) = 789.*cfc12(i,1)*dp(i,0)
         df22 (i,0) = 789.*cfc22(i,1)*dp(i,0)

         dh2o (i,0) = max(dh2o(i,0),1.e-10)
         do3  (i,0) = max(do3 (i,0),1.e-6)
         dco2 (i,0) = max(dco2(i,0),1.e-4)

         xx=pa(i,0)*0.001618*wa(i,1)*wa(i,1)*dp(i,0)
         dcont(i,0) = xx*exp(1800./ta(i,1)-6.081)
       enddo
...
c-----Index "0" is the layer above the top of the atmosphere.
        ...
        do i=1,m
          blayer(i,0)=blayer(i,1)
          blevel(i,0)=blayer(i,1)
        enddo
...
c     bs and dbs include the effect of surface emissivity.
          ...
          call sfcflux (ibn,m,ns,fs,tg,eg,tv,ev,rv,bs,dbs,rflxs) 
...
c     It is for a specific drop size distribution provided by Q. Fu.
     ...
     *        taucl(i,k,1) + taucl(i,k,2) + taucl(i,k,3)
        enddo
       enddo
...
c     a diffusivity factor of 1.66.
           ...
           enn    (i,k) = fcld(i,k)*(1.0-tcldlyr(i,k)) ! N in the documentation (6.20)
...
!mjs
       ...
       call mkicx   (m,np,ict,icb,enn,icx,ncld)
!mjs
...
ccef          idx = GetAeroIndex(aerosols(j), rc)

          do i=1,m
ccef            call Get_AeroOptProp(idx, ibn+8,qaero(i,k,j),rh(i,k),
...
c     and (6.12). 
           ...
           ff=.5+(.3739+(0.0076+0.1185*asya(i,k))*asya(i,k))*asya(i,k)
...
c-----blayer(i,np+1) includes the effect of surface emissivity.
      ...
      do i=1,m
        bu(i,0)=0.0 ! ALT: this was undefined, check with Max if 0.0 is good value
        bd(i,0)=blayer(i,1)
        bu(i,np+1)=blayer(i,np+1)
        au(i,0)=0.0 ! ALT: this was undefined, check with Max if 0.0 is good value
        ad(i,0)=blayer(i,1)
        au(i,np+1)=blayer(i,np+1)
        cu(i,0)=0.0 ! ALT: this was undefined, check with Max if 0.0 is good value
        cd(i,0)=blayer(i,1)
        cu(i,np+1)=blayer(i,np+1)
      enddo
...
!         dpe=0.2
     ...
     *                w13,p13,dwe,dpe,o1,o2,o3,trant)
      endif
...
c     trant(i,k2) is the total transmittance of the layer k2-1.
          
        do i=1,m
          xx=(1.-enn(i,k2-1))*trant(i,k2)
          yy=min(0.9999,xx)
          yy=max(0.00001,yy)
          ...
          yy=max(0.00001,yy)
          xx=(blevel(i,k2-1)-blevel(i,k2))/alog(yy)
          cd(i,k2-1)=(blevel(i,k2)-blevel(i,k2-1)*yy)/(1.0-yy)-xx
          cu(i,k2-1)=(blevel(i,k2-1)+blevel(i,k2))-cd(i,k2-1)
        enddo
        ...
        if(do_aerosol) then
           do i=1,m
              ...
              yy=max(0.00001,yy)
              xx=(blevel(i,k2-1)-blevel(i,k2))/alog(yy)
              ad(i,k2-1)=(blevel(i,k2)-blevel(i,k2-1)*yy)/(1.0-yy)-xx
              au(i,k2-1)=(blevel(i,k2-1)+blevel(i,k2))-ad(i,k2-1)
           enddo
        else
           do i=1,m
              ad(i,k2-1)=cd(i,k2-1)
...
!mjs
      if (.not. overcast) then
         call cldovlp (m,np,k1,k2,ict,icb,icx,ncld,enn,tcldlyr,
     *        cldhi,cldmd,cldlw)
...
c     flau, flad: clear-sky aerosol-free upward and downward fluxes
...
c-----The first terms on the rhs of Eqs. (8.15) and (8.16)
        ...
        if (k2.eq.k1+1 .and. ibn .ne. 10) then
         ...
         do i=1,m
          flau(i,k1)=flau(i,k1)-au(i,k1)
          flad(i,k2)=flad(i,k2)+ad(i,k1)
          flcu(i,k1)=flcu(i,k1)-cu(i,k1)
          flcd(i,k2)=flcd(i,k2)+cd(i,k1)
...
c     Also see Eqs. (5.4) and (5.5) for Band 10.
         ...
         do i=1,m
          ...
          xx=trant(i,k2)*(cu(i,k2-1)-cu(i,k2))
          flcu(i,k1) =flcu(i,k1)+xx

          if(do_aerosol) then
             xx = taant(i,k2)*(au(i,k2-1)-au(i,k2))
          endif
          flau(i,k1) =flau(i,k1)+xx

          if (k1.eq.0) then !mjs  bd(i,-1) is not defined
             xx=-trant(i,k2)*bd(i,k1)
          else
          ...
          if (k1.eq.0) then !mjs  bd(i,-1) is not defined
             xx=-trant(i,k2)*cd(i,k1)
          else
             xx= trant(i,k2)*(cd(i,k1-1)-cd(i,k1))
          endif
          flcd(i,k2) =flcd(i,k2)+xx

          if(do_aerosol) then
             if (k1.eq.0) then  !mjs  bd(i,-1) is not defined
                xx=-taant(i,k2)*ad(i,k1)
             else
                xx= taant(i,k2)*(ad(i,k1-1)-ad(i,k1))
             endif
          endif

          flad(i,k2) =flad(i,k2)+xx
...
c     Note: blayer(i,np+1) and dbs include the surface emissivity 
c     effect. Both dfdts and sfcem are negative quantities.

         do i=1,m
            flau(i,np+1) =         -blayer(i,np+1)
...
c     rhs of Eq. 8.16). rflxs is the surface reflectivity.

         do k=1,np+1
            do i=1,m
               flau(i,k) = flau(i,k)-flad(i,np+1)*trantca(i,k)*rflxs(i)
               flcu(i,k) = flcu(i,k)-flcd(i,np+1)*trantcr(i,k)*rflxs(i)
               flxu(i,k) = flxu(i,k)-flxd(i,np+1)*transfc(i,k)*rflxs(i)
            enddo
         enddo
...
c-----Summation of fluxes over spectral bands
      ...
      do k=1,np+1
         do i=1,m
            fla(i,k) = fla(i,k) + (flad(i,k)+flau(i,k))
...
c---- input parameters ------
      ...
      real dh2o(m,0:np),pa(m,0:np),dt(m,0:np)


c---- output parameters -----


      real h2oexp(m,0:np,6)
...
c---- input parameters -----
      ...
      real dco2(m,0:np),pa(m,0:np),dt(m,0:np)


c---- output parameters -----


      real co2exp(m,0:np,6,2)
...
c---- input parameters -----
      ...
      real dn2o(m,0:np),pa(m,0:np),dt(m,0:np)


c---- output parameters -----


      real n2oexp(m,0:np,4)
...
c---- input parameters -----
      ...
      real dch4(m,0:np),pa(m,0:np),dt(m,0:np)


c---- output parameters -----


      real ch4exp(m,0:np,4)
...
c---- input parameters -----
      ...
      real dcom(m,0:np),dt(m,0:np)


c---- output parameters -----


      real comexp(m,0:np,6)
...
c---- input parameters -----
      ...
      real dh2o(m,0:np),dcont(m,0:np),dn2o(m,0:np)
      real dco2(m,0:np),pa(m,0:np),dt(m,0:np)
...
c---- input parameters -----
      ...
      real w1,p1,dwe,dpe
      real dw(m,0:np),p(m,0:np),dt(m,0:np)
...
c---- update parameter -----
      ...
      real s1(m),s2(m),s3(m),tran(m,0:np+1)
...
c       we=(log10(x1)-w1)/dwe
c       pe=(log10(x2)-p1)/dpe
       we=(log10(x1)-w1)*dwe
       pe=(log10(x2)-p1)*dpe
...
c       pa = coef1(ip,iw-1)*(1.-fp)+coef1(ip+1,iw-1)*fp
c       pb = coef1(ip,  iw)*(1.-fp)+coef1(ip+1,  iw)*fp
c       pc = coef1(ip,iw+1)*(1.-fp)+coef1(ip+1,iw+1)*fp
...
c       ax = (-pa*(1.-fw)+pc*(1.+fw)) *fw*0.5 + pb*(1.-fw*fw)
...
c       ba = coef2(ip,  iw)*(1.-fp)+coef2(ip+1,  iw)*fp
c       bb = coef2(ip,iw+1)*(1.-fp)+coef2(ip+1,iw+1)*fp
c       t1 = ba*(1.-fw) + bb*fw
...
c       ca = coef3(ip,  iw)*(1.-fp)+coef3(ip+1,  iw)*fp
c       cb = coef3(ip,iw+1)*(1.-fp)+coef3(ip+1,iw+1)*fp
c       t2 = ca*(1.-fw) + cb*fw
...
c---- input parameters ------
      ...
      real conexp(m,0:np,3),h2oexp(m,0:np,6)
...
c---- updated parameters -----
      ...
      real th2o(m,6),tcon(m,3),tran(m,0:np+1)
...
c***********************************************************************
      subroutine cldovlp (m,np,k1,k2,ict,icb,icx,ncld,enn,ett,
     *                    cldhi,cldmd,cldlw)


c***********************************************************************
c
c     update the effective superlayer cloud fractions between  levels k1
c     and k2 following Eqs.(6.18)-(6.21). This assumes that the input
c     are the fractions between k1 and k2-1.
c
c input parameters
c
c  m:       number of soundings
c  np:      number of layers
c  k1:      index for top level
c  k2:      index for bottom level
c  ict:     the level separating high and middle clouds
c  icb:     the level separating middle and low clouds
c  icx:     indeces sorted by enn in the three superlayers
c  enn:     effective fractional cloud cover of a layer
c  ett:     transmittance of a cloud layer
c
c update paremeters
c
c  cldhi:   Effective high-level cloudiness
c  cldmd:   Effective middle-level cloudiness
c  cldlw:   Effective low-level cloudiness
c
c
c***********************************************************************
      ...
      integer, intent(IN   ) :: m,np,k1,k2,ict,icb,icx(m,0:np),ncld(m,3)
      real,    intent(IN   ) :: enn(m,0:np), ett(m,0:np)

      real,    intent(INOUT) :: cldhi(m),cldmd(m),cldlw(m)
 	...
 	if(enn(i,km)<0.001) cycle

        if    (km <  ict               ) then ! do high clouds

           kx=ncld(i,1)

           if(kx==1 .or. cldhi(i)==0.) then
              cldhi(i) = enn(i,km)
           else
              cldhi(i) = 0.0
              if(kx==0) cycle
              do k=ict-kx,ict-1
                 j=icx(i,k)
                 if(j>=k1.and.j<=km) cldhi(i)=enn(i,j)+ett(i,j)*cldhi(i)
              enddo
           endif
        ...
        elseif(km >= ict .and. km < icb) then ! do middle clouds
           ...
           if(kx==1 .or. cldmd(i)==0.) then
              ...
              do k=icb-kx,icb-1
                 j=icx(i,k)
                 if(j>=k1.and.j<=km) cldmd(i)=enn(i,j)+ett(i,j)*cldmd(i)
              enddo
           endif
           ...
           if(kx==1 .or. cldlw(i)==0.) then
              ...
              do k=np+1-kx,np
                 j=icx(i,k)
                 if(j>=k1.and.j<=km) cldlw(i)=enn(i,j)+ett(i,j)*cldlw(i)
              enddo
           endif
...
c***********************************************************************
      ...
      subroutine sfcflux (ibn,m,ns,fs,tg,eg,tv,ev,rv,bs,dbs,rflxs)
...
c---- temporary arrays -----
       ...
       integer i,j
       real tx(m),ty(m),bgx(m),bvx(m),dbgx(m),dbvx(m)
       real bg(m,ns),bv(m,ns),dbg(m,ns),dbv(m,ns)
       real xx,yy,zz
...
c     c.f. eqs. (3.11) and (3.12).
        ...
        do j=1,ns
         ...
         enddo
          ...
          call planck(ibn,m,tx,bgx)
          call planck(ibn,m,ty,bvx)
          call plancd(ibn,m,tx,dbgx)
          call plancd(ibn,m,ty,dbvx)
...
c-----
        ...
        do i=1,m
         ...
         if (fs(i,1).gt. 0.9999) then
          ...
          if (ev(i,1,ibn).lt.0.0001 .and. rv(i,1,ibn).lt.0.0001) then
...
c     following Eqs. (9.4), (9.5), and (3.13)
           ...
           bs(i) =eg(i,1,ibn)*bg(i,1)
           dbs(i)=eg(i,1,ibn)*dbg(i,1)
...
c     following Eqs. (9.1), (9.3), and (9.13)
           ...
           xx=ev(i,1,ibn)*bv(i,1)
           yy=1.0-ev(i,1,ibn)-rv(i,1,ibn)
           zz=1.0-eg(i,1,ibn)
           bs(i)=yy*(eg(i,1,ibn)*bg(i,1)+zz*xx)+xx


           xx=ev(i,1,ibn)*dbv(i,1)
           dbs(i)=yy*(eg(i,1,ibn)*dbg(i,1)+zz*xx)+xx
...
c-----No vegetation, following Eqs. (9.9), (9.10), and (9.13)
          ...
          do j=1,ns
            ...
            bs(i)=bs(i)+fs(i,j)*eg(i,j,ibn)*bg(i,j)
            dbs(i)=dbs(i)+fs(i,j)*eg(i,j,ibn)*dbg(i,j)
...
c-----With vegetation, following Eqs. (9.6), (9.7), and (9.13)
           ...
           do j=1,ns
            ...
            xx=ev(i,j,ibn)*bv(i,j)
            yy=1.0-ev(i,j,ibn)-rv(i,j,ibn)
            zz=1.0-eg(i,j,ibn)
            bs(i)=bs(i)+fs(i,j)*(yy*(eg(i,j,ibn)*bg(i,j)+zz*xx)+xx)


            xx=ev(i,j,ibn)*dbv(i,j)
            dbs(i)=dbs(i)+fs(i,j)*(yy*(eg(i,j,ibn)*dbg(i,j)+zz*xx)+xx)
...
!mjs
      ...
      subroutine mkicx(m,np,ict,icb,enn,icx,ncld)

      implicit none

      integer, intent(IN   ) :: m, np, ict, icb
      real,    intent(IN   ) :: enn(m,0:np)
      integer, intent(INOUT) :: icx(m,0:np)
      integer, intent(  OUT) :: ncld(m,3)
      ...
      integer i
       ...
       SOUNDINGS: do i=1,m
          ...
          call sortit(enn(i,  0:ict-1),icx(i,  0:ict-1),ncld(i,1),  0)
          call sortit(enn(i,ict:icb-1),icx(i,ict:icb-1),ncld(i,2),ict)
          call sortit(enn(i,icb:np   ),icx(i,icb:   np),ncld(i,3),icb)
       ...
       subroutine SORTIT(ENN,LST,NC,ibg)

       integer, intent(  OUT) :: NC
       integer, intent(IN   ) :: IBG
       real,    intent(IN   ) :: ENN(IBG:)
       integer, intent(INOUT) :: LST(IBG:)
