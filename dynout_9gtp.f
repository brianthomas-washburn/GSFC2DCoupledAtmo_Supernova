...
C
      if (iiyr .ge. 2) then
       ...
       ccf = '/misc/kah05/fleming/9gtrans/dyn_9gtp_'//ccyr(iiyr)//'.xdr'

       OPEN (463, FILE = ccf, convert="big_endian",
     >   form = 'binary', access='sequential', status = 'new')
            write (463) wall
            write (463) kyyall
            write (463) kzztot
            write (463) tempall
...
ccccctest9gt      IF (iiyr .eq. 2  .and.  im10 .eq. 72) THEN
 ...
 1413	   tempall(ij,ik,74) = tempall(ij,ik,73)
      ...
      ccf= '/misc/kah05/fleming/9gtrans/dyn_9gtp_'//ccyr(iiyr+1)//'.xdr'
...
C
 ...
 4920     tempcl(ij,ik,74) = tempcl(ij,ik,2)
      ...
      OPEN (463, FILE = '/misc/kah05/fleming/9gtrans/dyn_9gtp_clim.xdr',
