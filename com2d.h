...
c 	NMON$ = number of months for interannual run, usually NYR$*72
	...
	COMMON/DAT100/INTTRA(T$), ITPDF, ILPDF, IZONAVGT
...
c  commons for reaction rate data 
	...
	COMMON/CTRL/INDAYS,IOUTP, INYEARS
...
c  commons for photolysis calculations
	...
	!AYelland 10Oct2020 - used to determine if/when ionization data should be read-in
      INTEGER ionsflag, lightnflag, ionsflag_solv, lightnflag_solv,
     >   runNum, ionStart
      REAL*4 ionSource(L$,Z$), GCRions(L$,Z$), ionsRatio(L$,Z$)


!BThomas Dec2022 - used for control of additional H input source
	INTEGER Hinflag,Hinflag_solv,HinStart,HinStop
	COMMON /Hinput/ Hinflag,Hinflag_solv,HinStart,HinStop
	
	COMMON /NOXGCR/ NOXAIR(L$,Z$), GCR(L$,Z$), ionSource, GCRions, 
     > ionsRatio, ionsflag, lightnflag, ionsflag_solv, lightnflag_solv,
     > runNum, ionStart
	   !BThomas 24Jan2019 - added 'ionSource' here, see ionSourceIn.f
...
C  COMMONS FOR LIFETIME CALCULATION - updated for SPARC assessment
        COMMON/LIFE/CLOSS(40,10,L$),CLOSSST(40,10,L$),CLOSSTR(40,10,L$),
     >	   COLUMN(40,L$), RLOSS(40,L$,Z$), LIFECHAR(40), YSLFPRNT, IFIXOH

        COMMON/LIFE2/xlife(30,6,L$,Z$), xlifej(30,5,L$,Z$),
     >         CLOSSM(40,10,L$), CLOSSZ(40,10,L$,Z$), COLUMNZ(40,L$,Z$)
...
C
     ...
     >                 COLDENHNO3(L$,Z$) !BThomas Nov2017 -- added for HNO3 rainout
...
!BThomas Nov2017 -- adding some stuff used in additions I've made
      REAL*8 HNO3RAIN(L$,Z$)
